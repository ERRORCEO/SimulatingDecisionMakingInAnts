...
def RobinsonCode(n, quals, probs, threshold_mean, threshold_stddev, qual_stddev, time_means, time_stddevs, ToPlot, quora):
    ...
    for i in range(n): # note that Python indexing is from 0, whereas MATLAB is from 1
        ...
        #print(threshold_stddev, np.random.randn(), threshold_mean)
        #thresh = threshold_stddev * np.random.randn() + threshold_mean
        thresh = threshold_stddev * engine.random(n=1)[0][0] * random.choice([-1,1]) + threshold_mean #LATENTHYPERCUBEABLE
        #print("latranthres",engine.random(n=1)[0][0])
        ...
        while Ants[i]['selected'] == 0:
            ...
            #print(qual_stddev[accepts[i]], np.random.randn(), quals[accepts[i]])
            #perceivedQuality = qual_stddev[accepts[i]] * np.random.randn() + quals[accepts[i]]
            perceivedQuality = qual_stddev[accepts[i]] * engine.random(n=1)[0][0] * random.choice([-1,1]) + quals[accepts[i]] #LATENTHYPERCUBEABLE
            #print("latranqual",engine.random(n=1)[0][0])
            ...
            # between 0 and 1
            #ran = np.random.uniform() #MonteCarlo
            ran = engine.random(n=1)[0][0]                                                                                #LATENTHYPERCUBEABLE
            #print("initran",ran) #WORK FROM HERE
            ...
            while ran > probs[newsite, accepts[i]]:
                #print("beforeran", ran, "prob", probs[newsite, accepts[i]],"newsite", newsite)
                ran = ran - probs[newsite, accepts[i]]
                newsite = newsite + 1
                #print("afterran", ran, "prob", probs[newsite, accepts[i]],"newsite", newsite)
                #print("after selection",probs[newsite, accepts[i]], ran, newsite)
            ...
            delta = max(1, time_stddevs[newsite, accepts[i]] * engine.random(n=1)[0][0] + time_means[newsite, accepts[i]])  #LATENTHYPERCUBEABLE
            #print("latrandelta",engine.random(n=1)[0][0])
    ...
    #        end
        ...
        if (accepts[i] == 1):
            antSelectedOne.append(i)
        else:
            antSelectedTwo.append(i)
    ...
    return current_time, discovers, visits, accepts, Ants, rnd_seed, antSelectedOne, antSelectedTwo, [], [], [], [] # , preqtimes, preqdiscovers, preqvisits, preqaccepts
